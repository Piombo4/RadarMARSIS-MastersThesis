...
c = 299792458;
metodo = 3;
folder = 'D:\nuovi radargrammi da pulire\belli\';
files = dir(fullfile(folder, '*_M.DAT'));
results = table('Size',[0 3], ...
                'VariableTypes',{'string','double','double'}, ...
                'VariableNames',{'Orbita','SNRprima','SNRdopo'});
for i = 1:length(files)
    try
        FrmFile = fullfile(files(i).folder, files(i).name);
        ...
        % FFT
        phasegram = fft(complex_matrix, [], 1);
        phasegram = fftshift(phasegram, 1);

        banda_logica = abs(f) <= 0.5e6;
        phasegram(~banda_logica, :) = 0;

        phasegram = fft(phasegram, [], 2);
        phasegram = fftshift(phasegram, 2);
        ...
        switch metodo
            case 1
                energia_totale = sum(mediana_colonne);
                energia_cum = cumsum(mediana_colonne) / energia_totale;
        ...
        temp = ifftshift(phasegram_filtered, 2);
        temp = ifft(temp, [], 2);
        ...
        for j = 1:Nc
        col_b = abs(complex_matrix(:,j)).^2;  % potenza
        col_a = abs(newMatrix(:,j)).^2;
       ...
       sorted_b = sort(col_b, 'descend');
        ...
        snr_before(j) = 10*log10( (pk_b + eps) / noise_b );
        snr_after(j)  = 10*log10( (pk_a + eps) / noise_a );
        end

        snr_before_mean = mean(snr_before,'omitnan');
        snr_after_mean  = mean(snr_after,'omitnan');
        ...
        results = [results; {numero, round(snr_before_mean,2), round(snr_after_mean,2)}];

    catch ME
        fprintf('Errore su orbita: %s\n', ME.message);
    end
end
outputCSV = fullfile("./",'risultati.csv');
